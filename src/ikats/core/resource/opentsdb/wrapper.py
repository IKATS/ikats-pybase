...
class Wrapper(object):
    ...
    def create_tsuid(cls, fid, show_details=False):
        ...
        try:
            ...
            tsuid = cls._extract_uid_from_json(item_type='metric', value=metric, json=results)

            # retrieving and concatenating by pair [ tagk + tagv ] uids from opentsdb json response
            tagkv_items = [cls._extract_uid_from_json(item_type='tagk', value=str(k), json=results) +
                           cls._extract_uid_from_json(item_type='tagv', value=str(v), json=results)
    ...
    def inherit_properties(cls, tsuid, parent):
        ...
        try:
            ...
            for meta_name in metadata:
                if not Wrapper.NON_INHERITABLE_PATTERN.match(meta_name):
