...
from ikats.core.library.exception import IkatsNotFoundError, IkatsConflictError, IkatsException, IkatsInputError
from ikats.core.resource.client import RestClient
...
class TemporalDataMgr(RestClient):
    ...
    def import_ts_data(self, metric, data, fid, tags=None):
        ...
        if type(data) is np.ndarray:
            ...
            self.logger.debug("Creating file: %s", filename)
            ...
            self.logger.error("'data' must be a valid file path or a numpy array (got: %s %s)", type(data), data)
        ...
        if type(tags) is not dict:
            self.logger.error("tags must be a dict")
    ...
    def get_ts(self, tsuid_list, sd=None, ed=None):
        ...
        if type(tsuid_list) is not list:
            self.logger.error("get_ts: tsuid_list must be a list or str")
        ...
        for tsuid in tsuid_list:
            ...
            if sd is None:
                ...
                if 'ikats_start_date' in metadata[tsuid]:
                    ...
                    self.logger.warning("no 'ikats_start_date' meta data for ts %s", tsuid)
            ...
            if calc_dates:
                ...
                self.logger.info("Calculating date for %s during get_ts method", tsuid)
                self.logger.info("   'ikats_start_date' = %s", ikats_start_date)
                self.logger.info("   'ikats_end_date'   = %s", ikats_end_date)
                self.logger.info("   'qual_nb_points'   = %s", qual_nb_points)

                self.import_meta_data(tsuid=tsuid, name='ikats_start_date', value=ikats_start_date,
                                      data_type=DTYPE.date)
                self.import_meta_data(tsuid=tsuid, name='ikats_end_date', value=ikats_end_date, data_type=DTYPE.date)
                self.import_meta_data(tsuid=tsuid, name='qual_nb_points', value=qual_nb_points, data_type=DTYPE.number)
    ...
    def get_ts_by_tsuid(self, tsuid, sd, ed=None, ag='avg', old_format=False):
        ...
        if type(sd) != int:
            ...
            raise TypeError("sd must be a number (got: %s)" % sd)
        if sd < 0:
            self.logger.error("sd must be positive (got: %s)", sd)
        ...
        if ed is None:
            ...
            if type(ed) != int:
                self.logger.error("ed must be a number (got: %s)", ed)
                raise TypeError("ed must be a number (got: %s)" % ed)
            if ed < 0:
                self.logger.error("ed must be positive (got: %s)", ed)
                raise ValueError("ed must be positive (got: %s)" % ed)
            if ed < sd:
                self.logger.error("ed must be greater than sd (got: %s < %s)", ed, sd)
                raise ValueError("ed must be greater than sd (got: %s < %s)" % (ed, sd))
            if ed == sd:
                # ed should be greater than sd
                ed += 1
        if type(ag) != str:
            self.logger.error("ag must be a string (got: %s)", ag)
        ...
        # Filling query parameters
        uri_params = {
            "host": self.config_reader.get('cluster', 'opentsdb.read.ip'),
            "port": self.config_reader.get('cluster', 'opentsdb.read.port'),
            "sd": sd,
            "ed": ed,
            "ts_info": ts_info
        }
        ...
        response = self._send(
            verb=RestClient.VERB.GET,
            template='direct_extract_by_tsuid',
            uri_params=uri_params)
    ...
    def get_ts_info(self, metric, query_params=None):
        ...
        Example:
            ``tdm.get_ts_info("WS6",{'flightIdentifier':['625','629']})``
        will return:
            | [
            |     {
            |         'metric': 'WS6',
        ...
        if type(query_params) is not dict and query_params is not None:
            self.logger.error('query_params shall be a dict')
            raise TypeError('query_params shall be a dict')

        if type(metric) is not str:
            self.logger.error('metric shall be a str')
    ...
    def remove_ts(self, tsuid):
        ...
        if type(tsuid) is not str:
            self.logger.error("tsuid must be a string (got %s)", type(tsuid))
    ...
    def import_data_set(self, data_set_id, description, tsuid_list):
        ...
        if type(tsuid_list) is not list:
            self.logger.error('tsuid_list shall be a list')
            raise TypeError('tsuid_list shall be a list')
        ...
        data = {
            ...
            'tsuidList': ','.join(tsuid_list),
        }

        response = self._send(
            verb=RestClient.VERB.POST,
            template='import_data_set',
            uri_params=uri_params,
            data=data)
    ...
    def get_data_set(self, data_set):
        """
        Retrieve the list of TS identifier corresponding to a data set provided in arguments

        Corresponding web app resource operation: **getDataSet**

        :param data_set: name of the data set to request TS list from
        :type data_set: str
        ...
        # Checks inputs
        if type(data_set) is not str:
            self.logger.error("data_set must be a string (got %s)", type(data_set))
            raise TypeError("data_set must be a string (got %s)" % type(data_set))
        ...
        response = self._send(
            verb=RestClient.VERB.GET,
            template='get_data_set',
            uri_params=uri_params)

        if 'tsuidsAsString' in response.json:
            ret['ts_list'] = response.json['tsuidsAsString']
    ...
    def remove_data_set(self, data_set, deep=False):
        """
        Remove data_set from base

        Corresponding web app resource operation: **removeDataSet**

        :param data_set: name of the data set to delete
        :type data_set: str
        ...
        if type(deep) is not bool:
            self.logger.error("deep must be a boolean (got %s)", type(deep))
            raise TypeError("deep must be a boolean (got %s)" % type(deep))
        ...
        if deep:
            response = self._send(
                verb=RestClient.VERB.DELETE,
                template='deep_remove_data_set',
                uri_params=uri_params)
        else:
            response = self._send(
                verb=RestClient.VERB.DELETE,
                template='remove_data_set',
                uri_params=uri_params)
    ...
    def import_fid(self, tsuid, fid):
        ...
        if type(fid) is not str:
            self.logger.error("fid must be a string (got %s)", type(fid))
            raise TypeError("fid must be a string (got %s)" % type(fid))

        if tsuid == "":
            self.logger.error("tsuid must not be empty")
            raise ValueError("tsuid must not be empty")
        if fid == "":
            self.logger.error("fid must not be empty")
        ...
        if response.status == 200:
            self.logger.info("TSUID:%s - FID created %s", tsuid, fid)
        elif response.status == 409:
            self.logger.warning("TSUID:%s - FID already exists (not updated) %s", tsuid, fid)
            raise IndexError("TSUID:%s - FID already exists (not updated) %s" % (tsuid, fid))
        else:
            self.logger.warning("TSUID:%s - FID %s not created (got %s)", tsuid, fid, response.status)
    ...
    def get_fid(self, tsuid):
        ...
        if response.status == 200:
            ...
            if response.json != '{}':
                ...
                self.logger.debug("TSUID:%s - FID obtained %s", tsuid, fid)
                return fid
            else:
                self.logger.warning("No FID for TSUID [%s]", tsuid)
    ...
    def delete_fid(self, tsuid):
        ...
        # in case of success, web app returns 2XX
        if response.status == 200:
            self.logger.info("TSUID:%s - FID deleted", tsuid)
        else:
            self.logger.warning("TSUID [%s] - FID not deleted. Received status:%s", tsuid, response.status)
            raise ValueError
    ...
    def import_meta_data(self, tsuid, name, value, data_type=DTYPE.string, force_update=False):
        ...
        if type(name) is not str:
            self.logger.error("name must be a string (got %s)", type(name))
            raise TypeError("name must be a string (got %s)" % type(name))
        if type(value) not in [str, int, float]:
            self.logger.error("value must be a string or a number (got %s)", type(value))
            raise TypeError("value must be a string or a number (got %s)" % type(value))
        if type(data_type) is not DTYPE:
            self.logger.error("data_type must be a DTYPE (got %s)", type(data_type))
            raise TypeError("data_type must be a DTYPE (got %s)" % type(data_type))

        if type(force_update) is not bool:
            self.logger.error("force_update must be a bool (got %s)", type(force_update))
        ...
        if tsuid == "":
            ...
            raise ValueError("tsuid must not be empty")
        if name == "":
            self.logger.error("name must not be empty")
            raise ValueError("name must not be empty")
        if value == "":
            self.logger.error("value must not be empty")
        ...
        response = self._send(
            verb=RestClient.VERB.POST,
            template='import_meta_data',
            uri_params=uri_params,
            q_params=q_params)

        # in case of success, web app returns 2XX
        if response.status == 200:
            self.logger.info("TSUID:%s - MetaData created %s(%s)=%s", tsuid, name, data_type, value)
            return True
        elif response.status == 409:
            if force_update:
                # Error occurred (can't create a metadata that already exists - conflict)
                # Try to update it because it is wanted
                self.logger.info("TSUID:%s - MetaData updated %s=%s", tsuid, name, value)
                return self.update_meta_data(tsuid=tsuid, name=name, value=value, force_create=False)
            else:
                self.logger.warning("TSUID:%s - MetaData already exists (not updated) %s=%s", tsuid, name, value)
                return False
        self.logger.warning("TSUID [%s] - MetaData not created %s=%s. Received status:%s", tsuid, name, value,
                            response.status)
        return False
    ...
    def update_meta_data(self, tsuid, name, value, data_type=DTYPE.string, force_create=False):
        ...
        if type(force_create) is not bool:
            self.logger.error("force_create must be a bool (got %s)", type(force_create))
        ...
        if response.status == 200:
            ...
            if force_create:
                ...
                self.logger.info("TSUID:%s - MetaData created %s=%s", tsuid, name, value)
                return self.import_meta_data(tsuid=tsuid, name=name, value=value, data_type=data_type,
                                             force_update=False)
            else:
                self.logger.warning("TSUID:%s - MetaData doesn't exists and not Created %s=%s", tsuid, name, value)
                return False
        self.logger.warning(
            "TSUID [%s] - MetaData not updated %s=%s. Received status:%s", tsuid, name, value, response.status)
        return False
    ...
    def get_meta_data(self, ts_list):
        ...
        if type(ts_list) is not list:
            self.logger.error("ts_list must be a list")
        ...
        for i in range(0, len(ts_list), chunk_size):
            ...
            response = self._send(
                verb=RestClient.VERB.GET,
                template='lookup_meta_data',
                q_params=q_params)

            if response.status == 414:
                # The size of the request is too big
                # Decrease the chunk_size above
                self.logger.error("The size of the request is too big. Contact administrator")
                assert True
    ...
    def get_ts_from_meta_data(self, constraint=None):
        ...
        if type(constraint) is not dict:
            self.logger.error("constraint must be a dict")
    ...
    def get_func_id_from_tsuid(self, tsuid):
        ...
        :rtype: dict having following keys defined:
          ...
          - and 'funcId'
        :raises exception:
            - TypeError: if tsuid is not a str OR status 400 (bad request) OR unexpected http status
        ...
        response = self._send(
            verb=RestClient.VERB.GET,
            template='get_one_functional_identifier',
    ...
    def get_tsuid_from_func_id(self, func_id):
        ...
        :rtype: str
        :raises exception:
          - TypeError: if unexpected func_id parameter OR status 400 (bad request) OR unexpected http status
          - ValueError: mismatched result: http status 404:  not found
          - ServerError: http status for server errors: 500 <= status < 600
        """
    ...
    def search_functional_identifiers(self, criterion_type, criteria_list):
        ...
        self.logger.debug("search_functional_identifier_list with prepared Filter: %s", str(my_filter))

        response = self._send(
            verb=RestClient.VERB.POST,
            template='search_functional_identifier_list',
            uri_params=None,
            data=my_filter,
            files=None)
    ...
    def __check_fid_errors(self, response):
        ...
        :raises exception:
            - TypeError: if status 400 (bad request) OR unexpected http status
            - ValueError: mismatched result: http status 404:  not found
            - ServerError: http status for server errors: 500 <= status < 600
        """

        if response.status == 200:
            self.logger.debug('200 (ok) %s produced: %s ', response.url, response.json)
        elif response == 400:
            ...
            err_msg = "%s (unexpected status here) %s produced: %s" % (response.status,
                                                                       response.url,
                                                                       response.json)
